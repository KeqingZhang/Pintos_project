                 ...
                 void *aux UNUSED)
{
  ...
  return a->ticks_blocked < b->ticks_blocked;
}
...
void thread_sleep(int64_t ticks)
{
  ...
  cur->ticks_blocked = ticks + timer_ticks();
...
/*
  线程t需要一个锁，但是这个锁被其他线程拥有，所以我需要将我的优先级捐赠给那个线程（当那个线程优先级低的时候），使他赶紧结束。
  在捐赠的时候需要给那个线程自己的优先级捐赠，同时还得给他拥有的最高的锁（占据的最高优先级的资源）捐赠，使那个线程迅速结束。
  但是，在那个线程中，也需要等待别的锁，所以使用while进行了嵌套捐赠操作。
*/
...
void lock_update_priority(struct lock *l)
{
  ...
  struct thread *t;
  struct list_elem *e;
  for (e = list_begin(&l->waiters); e != list_end(&l->waiters); e = list_next(e))
  {
